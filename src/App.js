...
export default class App {
    ...
    #handleWebhook(req, res) {
        const exactSubstringsToRemove = [
            ...
            /\bPAGAMENTO\s+POS\b.*?\bPRESSO\b\s+/i,  // Rimuove tutto da "PAGAMENTO POS" fino a "PRESSO" incluso
            /\(CTV\.\s*(.*)/,
            ...
            /\bCARTA\b.*$/i,
            /\-\s*Transazione\s*C-less\b.*/i // Match "transazione C-less as the last word with any number of spaces between words and with any character after it
        ];
        ...
        if (!req.body.content.transactions[0].payment_date && !req.body.content.transactions[0].date) {
            throw new WebhookException('Missing content.transactions[0].payment_date or date');
        }
        ...
        const date = req.body.content.transactions[0].payment_date || req.body.content.transactions[0].date;
